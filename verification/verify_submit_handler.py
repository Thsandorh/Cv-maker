...
def run():
    with sync_playwright() as p:
        ...
        # Check if listener attached log is present
        listener_attached = any("CV Form submit listener attached" in log for log in logs)
        print(f"Listener attached log found: {listener_attached}")
        if not listener_attached:
            print("ALL LOGS:", logs)
        assert listener_attached, "Submit listener was not attached on load!"
